...
#   - Ind_0723.RData / TUS_0723.RData     (US 2007-2023)
...
preparar_dfAnalisis <- function(dfTus, dfInd, etiqueta) {
  ...
  cat("Distribución ANTES de recalificar interrupciones > 120 min:\n")
  print(round(prop.table(table(dfSurv$tipo)) * 100, 2))

  dfSurv <- dfSurv %>%
    mutate(
      tipo = case_when(
        tipo %in% c("Pure Interruption","Mixed Interruption") & duracion > 120 ~ "Censored",
        TRUE ~ tipo
      ),
...
# ==================================================================================================
# ----[ 1. ANÁLISIS US 2007-2023 ]------------------------------------------------------------------
# ==================================================================================================

cat("\n\n========== ANÁLISIS ESTADOS UNIDOS 2007-2023 ==========\n")

load(file.path(DATA_DIR, "TUS_0723.RData"))
load(file.path(DATA_DIR, "Ind_0723.RData"))

res_us <- preparar_dfAnalisis(dfTus_0723, Ind_0723, "US 2007-2023")
dfA    <- res_us$df
...
for (km in km_vars) {
  ...
  local({
    ...
    tryCatch({
      df_km <- dfA
      fmla  <- as.formula(paste("Surv(tiempo, estatus) ~", km_local$var))
      fit   <- survfit(fmla, data = df_km)
      p     <- ggsurvplot(fit, data = df_km, pval = TRUE, conf.int = TRUE,
                          risk.table = FALSE, ylim = c(0.7, 1),
                          xlab = "Time (Minutes)",
                          ylab = "Probability of no interruption",
                          legend.title = km_local$title,
                          legend.labs  = if (!is.null(km_local$labs)) km_local$labs else waiver(),
                          ggtheme = themePaper())
      fname <- paste0("fig_km_us_", tolower(gsub("[ /]","_", km_local$var)), ".pdf")
      save_gg(p$plot, fname, w = 10, h = 7)
      cat("Log-rank test:\n")
      print(survdiff(fmla, data = df_km))
    }, error = function(e) cat("KM error [", km_local$title, "]:", e$message, "\n"))
  })
}
...
tryCatch({
  ...
  top_ale <- if (!is.null(shap_vals)) head(shap_df$Variable, 8) else
    c("ageChild","male","nChild","educaGroup","hhldSize","familyStatus","age","income")
...
tryCatch({
  ...
  c_index_gbm <- concordance(Surv(dfGBM$tiempo, dfGBM$estatus) ~ lp_gbm)$concordance
...
tryCatch({
  ...
  pec_res <- pec(
    list(Cox=modeloCox, Cox_Strat=modeloCoxStrat, RSF=model_rsf_opt),
...
# ==================================================================================================
# ----[ 2.3 COX POOLED CON EFECTOS FIJOS DE PAÍS ]--------------------------------------------------

cat("\n\n--- COX POOLED con efectos fijos de país ---\n")

dfM <- dfM[!is.na(dfM$ocombwt) & dfM$ocombwt > 0, ]
dfM$country <- relevel(factor(dfM$country), ref="United States")

modeloCoxPooled <- coxph(
  Surv(tiempo, estatus) ~ male + age + midWeek + hhldSize + nChild + ageChild +
    urban + income + familyStatus + educaGroup + country + cluster(hldid_univ),
  weights = ocombwt,
  data    = dfM,
  x       = TRUE
)
cat("\n--- Resultados Cox pooled ---\n")
print(summary(modeloCoxPooled))
...
country_hr <- lapply(paises_ok, function(ctry) {
  df_c <- dfM[dfM$country == ctry & !is.na(dfM$ocombwt) & dfM$ocombwt > 0, ]
  if (sum(df_c$estatus) < 20) return(NULL)
  tryCatch({
    fit_c <- coxph(
      Surv(tiempo, estatus) ~ male + age + midWeek + hhldSize + nChild + ageChild +
        urban + income + educaGroup + cluster(hldid_univ),
      weights = ocombwt,
      data    = df_c
    )
    cf  <- coef(fit_c)["male1"]
    se  <- sqrt(vcov(fit_c)["male1","male1"])
    n_e <- sum(df_c$estatus)
    data.frame(
      country = ctry,
      HR      = exp(cf),
...
country_hr <- do.call(rbind, Filter(Negate(is.null), country_hr))

if (!is.null(country_hr) && nrow(country_hr) > 0) {
  cat("\n--- HR por género según país ---\n")
  print(country_hr[order(country_hr$HR), ])

  p_forest <- ggplot(country_hr, aes(x=reorder(country, HR), y=HR, ymin=lower, ymax=upper)) +
    geom_pointrange(size=0.7, color="steelblue") +
    geom_hline(yintercept=1, linetype="dashed", color="red") +
    coord_flip() +
    scale_y_log10() +
    labs(title="Hazard Ratio (male vs female) by country",
         subtitle="Cox model with controls. Reference = HR 1 (no gender difference)",
         x=NULL, y="HR (log scale) — values < 1: men less likely to interrupt") +
    themePaper() +
...
cat("\n\n--- RSF MULTI-PAÍS (500 árboles, con pesos) ---\n")

vars_rsf_m <- c("tiempo","estatus","male","age","midWeek","hhldSize","nChild","ageChild",
                "urban","income","cohab","citizen","educaGroup","familyStatus","country")
dfM_rsf <- dfM[, intersect(vars_rsf_m, names(dfM))]

tryCatch({
  set.seed(11082025)
  model_rsf_multi <- rfsrc(
    Surv(tiempo, estatus) ~ .,
    data     = dfM_rsf,
    case.wt  = dfM$ocombwt,
    ntree    = 500,
  ...
  cat("\n--- RSF multi-país ---\n")
  print(model_rsf_multi)

  save_base({
    par(mfrow = c(1, 2))
    plot(model_rsf_multi)
  }, "fig_rsf_multi_vimp.pdf", w = 14, h = 7)

  # ALE para variables clave del modelo multi-país
  X_multi <- dfM_rsf[, setdiff(names(dfM_rsf), c("tiempo","estatus"))]
  set.seed(42)
  idx_m <- sample(nrow(X_multi), min(2000, nrow(X_multi)))
  X_multi_s <- X_multi[idx_m, ]

  pfun_multi <- function(object, newdata) as.numeric(predict(object, newdata=newdata)$predicted)

  pred_multi <- Predictor$new(
    model            = model_rsf_multi,
    data             = X_multi_s,
    predict.function = function(model, newdata) pfun_multi(model, newdata)
  )

  for (v in c("country","male","ageChild","nChild","educaGroup")) {
    if (!(v %in% names(X_multi_s))) next
    tryCatch({
      ale_m <- FeatureEffect$new(pred_multi, feature=v, method="ale")
      p_ale_m <- ale_m$plot() +
        ggtitle(paste("ALE multi-country:", v)) +
        ylab("Effect on mortality") +
        themePaper()
      save_gg(p_ale_m, paste0("fig_ale_multi_", v, ".pdf"), w = 8, h = 6)
      cat("ALE multi-país:", v, "completado\n")
    }, error = function(e) cat("ALE multi error [", v, "]:", e$message, "\n"))
  }
}, error = function(e) cat("RSF multi-país error:", e$message, "\n"))
